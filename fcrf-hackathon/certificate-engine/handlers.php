...
function handle_verify(): void {
    ...
    $st = $pdo->prepare('SELECT 1 FROM allowed_attendees WHERE email = ? LIMIT 1');
    $st->execute([$email]);
    if (!$st->fetchColumn()) json_out(['ok' => false, 'reason' => 'not_registered']);

    $st = $pdo->prepare('SELECT name FROM certificate_claims WHERE email = ? LIMIT 1');
    $st->execute([$email]);
    ...
    try {
        $ins = $pdo->prepare('INSERT INTO certificate_claims (email, name) VALUES (?, ?)');
...
function handle_download(): void {
    ...
    if (!$grid) {
        try {
            db()->prepare('UPDATE certificate_claims SET download_count = download_count + 1, last_download_at = NOW() WHERE email = ?')
                ->execute([$cert['email']]);
